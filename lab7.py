...
def save_results(results):
    with open('results7.csv', 'w', newline='') as csvfile:
    ...
    while isInfecting:
        ...
        for i in range(0, Graph.vcount(g)):
            ...
            if len(neighborstab) > 0:
                ...
                for j in neighborstab:
                    if g.vs[j]["infected"] == 1:
                        infectedneighbors += 1

            if infectedneighbors >= options['social_influence'] * len(neighborstab):
                g.vs[i]["infected"] = 1
                g.vs[i]["stepinfected"] = step
        ...
        if print_flag == True:
            print("All nodes:", Graph.vcount(g), "Active nodes:", Graph.vcount(g) - removed, "Edges:", Graph.ecount(g),
                  "Step:", step,
                  ...
                  "Total infected:", infected, "Infected this step:", infected_step,
        ...
        results.append({
            ...
            'influence': options['social_influence']
        })
        if step >= 100 or infected == Graph.vcount(g) or infected_step == 0:
...
def main():
    if os.path.exists('results7.csv'):
        os.remove('results7.csv')

    g = Graph.Read_Ncol('facebook_combined.txt', directed=False)
    init_network(g)

    ## Próg aktywacji / szansa zarażenia
    pp_list = [1]  # b prog aktywacji

    ## Próg wyzdrowienia
    rec_list = [0.1]  # m rec

    ## Początkowy procent zarażonej populacji
    seed_list = [5, 10]  # n wezlow
    # pok = [0.1, 0.2, 0.3, 0.4]

    influence_list = [0.1, 0.3]
