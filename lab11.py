...
def save_results(results):
    with open('results11.csv', 'w', newline='') as csvfile:
...
## Inicjacja sieci
def init_network(g, available_chance = 0, reset = False):
    deg = []
    ...
    for i in range(0, Graph.vcount(g)):
        ...
        if round((random.uniform(0, 1)), 2) <= available_chance:
            g.vs[i]["available"] = 0

def simulation_loop(g, options, loop, results, set, debug = False, print = False):
                    ...
                    while isInfecting:
                        ...
                        results.append({
                            ...
                            'healthy': Graph.vcount(g) - removed - active_spreaders,
                            'available_chance': options['available']
                        })
...
def main():
    ...
    if os.path.exists('results11.csv'):
        os.remove('results11.csv')
    ...
    pp_list = [0.1]  # b prog aktywacji

    ## Próg wyzdrowienia
    rec_list = [0.1]  # m rec

    ## Początkowy procent zarażonej populacji
    seed_list = [5]  # n wezlow
    # pok = [0.1, 0.2, 0.3, 0.4]

    remove_list = [0.05]

    available_list = [0, 0.05, 0.1, 0.2]
            ...
            for seed_ratio in seed_list:
                for remove_chance in remove_list:
                    for available_chance in available_list:
                        ...
                        while loop <= 10:
                            init_network(g, available_chance = available_chance, reset = True)
